...
class _SignupScreenState extends State<SignupScreen> {
  ...
  Widget _birthDateWidget() { // 일단 텍스트 입력으로 만들어 놨는데 나중에 spinner ui 사용하는게 더 이쁠 것 같음.
    return TextFormField(
  ...
  Widget _phoneNumberWidget() { // 예외처리나 ui 손을 봐야함. 인증은 다음에 하는 걸로...
    return TextFormField(
      controller: _phoneNumberController,
      decoration: const InputDecoration(
          border: OutlineInputBorder(),
          labelText: '전화번호 010xxxxyyyy'
      ),
      validator: (String? value) {
        if(value!.isEmpty) {
          return '전화번호를 입력해주세요.';
    ...
    if(_formKey.currentState!.validate()) {
      ...
      try {
        ...
        userCollection.doc(user!.uid).set({
          ...
          'address': _addressController.text + " " + _specificAddressController.text,
          'profileUrl': "",
        }).then((value) async {
